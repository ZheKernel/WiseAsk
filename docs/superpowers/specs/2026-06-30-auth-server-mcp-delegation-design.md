...
| Ragent | OAuth Client ID 和客户端凭证 | 向 Auth Server 证明自己是受信任客户端 |
| Ragent | 当前请求的 Sa-Token | 作为 Token Exchange 的 `subject_token` |
| Auth Server | JWT RSA 私钥 | 唯一有权签发 Order MCP Access Token |
| Auth Server | Ragent Client 配置 | 校验允许申请令牌的客户端和 Scope |
| Auth Server | Sa-Token Redis 会话读取能力 | 验证 `subject_token` 并得到登录用户 ID |
...
```http
POST /oauth2/token
Authorization: Basic <ragent-client-credentials>
...
auth-server:
  ...
  ragent-client:
    ...
    client-secret: ${RAGENT_OAUTH_CLIENT_SECRET}
  audiences:
    order-mcp:
      user-scopes:
        - order:read:self
...
Ragent 能读取用户的 Sa-Token 并发起 Token Exchange，因此仍属于高信任服务。后续可将
Client Secret 升级为 `private_key_jwt` 或 mTLS，并把浏览器登录迁移到 Auth Server，
进一步减少共享状态和静态凭证。
