...
- `auth-server`：验证 Ragent `private_key_jwt`、用 Redis 拒绝重复 `jti`、交换
  Sa-Token 会话、签发 RS256 Access Token 并暴露 JWKS；同时拥有服务端 Scope
  授予规则。
- `mcp-order-server`：独立 PostgreSQL 订单查询服务，在 MCP 服务端再次校验角色
  和订单数据归属，并持有由已验证 JWT 构建的调用者身份模型。
...
订单 MCP 采用双重权限校验。Ragent 在调用前校验工具权限，并使用独有私钥向
`auth-server` 认证；Auth Server 验证当前 Sa-Token 和用户角色后签发 Audience 绑定的
短期 RS256 Access Token。`mcp-order-server` 通过 JWKS 验证令牌后，仍在服务端执行
Scope、角色和 SQL 行级过滤。模型参数不能决定调用者身份，普通用户无法通过传入其他
`userId` 查询他人订单。部署和验证步骤见 `docs/order-mcp-quick-start.md`。

OAuth Scope 字符串属于跨服务协议，各服务在自己的边界内维护同名常量：
`auth-server` 负责授予，`bootstrap` 负责请求，`mcp-order-server` 负责校验。不要让
Ragent 或资源服务依赖可执行的 `auth-server` 模块。
