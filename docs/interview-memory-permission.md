...
Ragent 当前 LoginUser
  ...
  -> 校验 signature / issuer / audience / time
  -> McpCallerIdentity 写入 TransportContext
  -> OrderMcpExecutor / OrderQueryService 再鉴权
  -> OrderRepository 参数化 SQL 行级过滤
```

JWT 包含 `sub`、`username`、`role`、`iss`、`aud`、`iat`、`nbf`、`exp` 和 `jti`。模型只能生成工具参数，不能生成或覆盖已签名的调用者身份。
...
| `order_admin_search` | Ragent 配置为管理员工具，订单服务再次校验管理员角色 |

订单服务只暴露固定、参数化、只读 SQL，并限制查询条数。启动发现使用 `system` 身份，它可以执行 `tools/list`，但会被订单业务层拒绝执行查询。
