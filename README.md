...
> 维护者：**cz**
>
> 本仓库基于 [nageoffer/ragent](https://github.com/nageoffer/ragent) 进行二次开发，重点增强会话记忆、长期记忆、Prompt 上下文编排、用户级资源隔离和受保护 MCP 调用能力。
...
本项目保留上游 Ragent 的文档入库、意图识别、多路检索、MCP 调用和流式问答能力，我主要围绕 Agent Harness 中的**记忆与上下文层**、**权限控制层**做了两组可独立验证的增强。
...
新增共享模块 `mcp-auth` 和独立服务 `mcp-order-server`，将权限控制延伸到进程边界：

```text
用户通过 Sa-Token 登录 Ragent
  -> Ragent 检查当前角色是否允许调用目标 MCP 工具
  -> 使用 HMAC-SHA256 签发短期内部 JWT
  -> JWT 携带 sub、username、role、issuer、audience、过期时间等声明
  -> MCP HTTP Client 写入 Authorization: Bearer <token>
  -> order MCP 校验签名、签发方、受众和有效期
...
Ragent 在工具执行前进行第一层策略检查，订单服务在收到请求后进行第二层令牌、角色和数据归属校验。即使有人绕过主应用直接请求订单 MCP，也无法在没有有效内部身份的情况下访问数据库。订单服务不开放任意 SQL，查询均为固定参数化 SQL，结果数量限制在 `1..100`。

服务启动时使用仅用于工具发现的短期 `system` 身份；该身份可以完成 MCP 初始化和 `tools/list`，但不能执行订单查询。详细初始化和启动步骤见 `docs/order-mcp-quick-start.md`。
