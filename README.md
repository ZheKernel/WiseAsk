...
新增独立 `auth-server` 和 `mcp-order-server`，将权限控制延伸到进程边界：
...
OAuth Scope 是跨服务协议值：`auth-server` 负责授予，`bootstrap` 负责请求，
`mcp-order-server` 负责校验。调用者身份模型只存在于订单资源服务内部，避免
Ragent 和 Order MCP 反向依赖可执行的授权服务。
