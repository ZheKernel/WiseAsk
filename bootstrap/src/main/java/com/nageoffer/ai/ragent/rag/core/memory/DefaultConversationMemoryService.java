...
public class DefaultConversationMemoryService implements ConversationMemoryService {
    ...
    @Override
    public ConversationMemoryContext loadContext(String conversationId, String userId) {
        // 参数校验
            ...
            return CompletableFuture.allOf(summaryFuture, historyFuture)
                    .thenApply(v -> {
                        ...
                        log.debug("加载对话记忆 - conversationId: {}, userId: {}, 摘要: {}, 历史消息数: {}, 耗时: {}ms",
                                conversationId, userId, StrUtil.isNotBlank(summary), history.size(), System.currentTimeMillis() - startTime);
                        return new ConversationMemoryContext(summary, history);
                    })
                    .join();
        } catch (Exception e) {
     ...
     * 加载摘要，失败时返回 null
     */
    private String loadSummaryWithFallback(String conversationId, String userId) {
        try {
